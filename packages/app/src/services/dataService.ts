...
interface ICharacterProfileRoot {
    characters: Record<string, ICharacterProfileEntry>;
}

interface IRelationsRoot {
    relations: Record<string, ICharacterRelationEntry[]>;
}

interface ITimelineRoot {
    markers: Record<string, [TimelineName, number]>;
}
...
interface IEntityLinksRoot {
    // [link, site, name?]
    links: Record<string, Array<[string, string, string?]>>;
}

interface IEntityLookupRoot {
    entries: Array<[string, EntityLookupEntityEntry[]]>;
}
...
export class DataService {
    ...
    public getLinksFor(entityId: RdfQName): Array<Readonly<IEntityLink>> {
    ...
    public lookupEntity(keyword: string, limit: number): IEntityLookupResultItem[] {
        ...
        for (const [kw, kwEntries] of this.entityLookup.entries) {
            ...
            for (const [qName, language, priority] of kwEntries) {
                ...
                const score = matchPriority * 10 + languageSimilarity * 5 + priority;
