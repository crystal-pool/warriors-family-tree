...
import scss from "./CharacterFamilyTree.module.scss";
...
function walk(characterId: RdfQName, walkMode?: CharacterFamilyTreeWalkMode, maxDistance?: number): IFamilyTreeData | null {
    ...
    const q: Array<[number, RdfQName, number, RdfQName?]> = [[0, characterId, maxDistance ?? -1]];
    const visited = new Set<RdfQName>();
    const mates = new Set<string>();
    const children: Array<[RdfQName, RdfQName | undefined, RdfQName]> = [];
...
export const CharacterFamilyTree: React.FC<ICharacterFamilyTreeProps> = React.memo((props) => {
    ...
    const renderNode: NodeRenderCallback = React.useCallback((id, _brct) => {
    ...
    >{familyTreeData
        ...
        : props.emptyPlaceholder ?? (<h3>{resourceManager.getPrompt("NoFamilyTreeInformation")}</h3>)}</Paper>);
});
