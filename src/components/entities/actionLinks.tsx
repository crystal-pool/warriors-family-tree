...
function renderActionLinks(actions: ActionLinkEntry[], displayAs?: IActionLinksProps["displayAs"]) {
    displayAs = displayAs || "link";
    return <ul className={classNames(Scss.actionLinks, Scss[displayAs])}>{actions.map(([label, href], i) => {
...
export const CharacterActionLinks: React.FC<IActionLinksProps> = function CharacterActionLinks(props) {
    ...
    return renderActionLinks(actions, props.displayAs);
};
